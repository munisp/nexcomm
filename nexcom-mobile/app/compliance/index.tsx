...
export default function ComplianceScreen() {
  ...
  const { data, isLoading, refetch } = trpc.compliance.getAlerts.useQuery(
    undefined,
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Compliance & AML</Text>
        ...
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          <View style={styles.cardGrid}>
            <View style={styles.statCard}>
              <Text style={styles.statValue}>{(data as any)?.['totalAlerts'] ?? '—'}</Text>
              <Text style={styles.statLabel}>Total Alerts</Text>
            </View>
            <View style={styles.statCard}>
              <Text style={styles.statValue}>{(data as any)?.['highRisk'] ?? '—'}</Text>
              <Text style={styles.statLabel}>High Risk</Text>
            </View>
            <View style={styles.statCard}>
              <Text style={styles.statValue}>{(data as any)?.['resolved'] ?? '—'}</Text>
              <Text style={styles.statLabel}>Resolved</Text>
            </View>
            <View style={styles.statCard}>
              <Text style={styles.statValue}>{(data as any)?.['pending'] ?? '—'}</Text>
