...
export default function AbcpScreen() {
  const [tab, setTab] = useState<'overview' | 'list'>('overview');

  const { data, isLoading, refetch } = trpc.abcp.getPrograms.useQuery(
    undefined,
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>ABCP Markets</Text>
        ...
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          <View style={styles.cardGrid}>
            <View style={styles.statCard}>
              <Text style={styles.statValue}>{(data as any)?.['activePrograms'] ?? '—'}</Text>
              <Text style={styles.statLabel}>Active Programs</Text>
            </View>
            <View style={styles.statCard}>
              <Text style={styles.statValue}>₦{(data as any)?.['totalIssued'] ?? '—'}</Text>
              <Text style={styles.statLabel}>Total Issued</Text>
            </View>
            <View style={styles.statCard}>
              <Text style={styles.statValue}>{(data as any)?.['avgRate'] ?? '—'}%</Text>
              <Text style={styles.statLabel}>Avg Rate</Text>
            </View>
            <View style={styles.statCard}>
              <Text style={styles.statValue}>{(data as any)?.['maturity'] ?? '—'}d</Text>
