...
impl SettlementEngine {
    ...
    /// Get settlement statistics for Prometheus metrics
    pub fn stats(&self) -> SettlementStats {
        ...
        for s in self.settlements.values() {
            match s.status {
                Status::Initiated | Status::PendingLedger | Status::PendingMojaloop | Status::PendingBlockchain => pending += 1,
                Status::Settled => completed += 1,
                Status::Failed | Status::Reversed => failed += 1,
            }
        }
        SettlementStats { pending, completed, failed }
    }
}
