/**
 * NEXCOM Exchange — fixedIncomeRouter Unit Tests
 *
...
describe("fixedIncomeRouter", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("router is defined", () => {
    expect(fixedIncomeRouter).toBeDefined();
  });
  it("list procedure exists", () => {
    expect(fixedIncomeRouter.list ?? fixedIncomeRouter.getAll ?? fixedIncomeRouter).toBeDefined();
  });
  it("has procedures", () => {
    expect(Object.keys(fixedIncomeRouter)).not.toHaveLength(0);
  });
});
