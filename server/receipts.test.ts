/**
 * NEXCOM Exchange — receiptsRouter Unit Tests
 *
...
describe("receiptsRouter", () => {
  ...
  it("list procedure exists", () => {
    expect(receiptsRouter.list ?? receiptsRouter.getAll ?? receiptsRouter.myReceipts ?? receiptsRouter).toBeDefined();
  });
  it("has at least 4 procedures", () => {
    expect(Object.keys(receiptsRouter).length).toBeGreaterThanOrEqual(4);
  });
});
