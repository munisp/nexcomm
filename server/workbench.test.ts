/**
 * NEXCOM Exchange — workbenchRouter Unit Tests
 *
...
describe("workbenchRouter", () => {
  ...
  it("router is defined", () => {
    expect(workbenchRouter).toBeDefined();
  });
  it("listFarms procedure exists", () => {
    expect(workbenchRouter.listFarms).toBeDefined();
  });
  it("summary procedure exists", () => {
    expect(workbenchRouter.summary).toBeDefined();
  });
  it("has at least 8 procedures", () => {
    expect(Object.keys(workbenchRouter).length).toBeGreaterThanOrEqual(8);
  });
});
