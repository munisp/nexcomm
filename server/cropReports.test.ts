/**
 * NEXCOM Exchange — cropReportsRouter Unit Tests
 *
...
describe("cropReportsRouter", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("router is defined", () => {
    expect(cropReportsRouter).toBeDefined();
  });
  it("has procedures", () => {
    expect(Object.keys(cropReportsRouter)).not.toHaveLength(0);
  });
});
