/**
 * NEXCOM Exchange — livePricesRouter Unit Tests
 *
...
describe("livePricesRouter", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("router is defined", () => {
    expect(livePricesRouter).toBeDefined();
  });
  it("getAll procedure exists", () => {
    expect(livePricesRouter.getAll).toBeDefined();
  });
  it("feedStatus procedure exists", () => {
    expect(livePricesRouter.feedStatus).toBeDefined();
  });
  it("has at least 5 procedures", () => {
    expect(Object.keys(livePricesRouter).length).toBeGreaterThanOrEqual(5);
  });
});
