  ...
  adminRunPerformanceReportsNow: adminProcedure
    .mutation(async ({ ctx }) => {
      ...
      for (const { obligation, profile } of activeObligations) {
        ...
        const [existing] = await db.select({ id: marketMakerPerformanceReports.id })
          .from(marketMakerPerformanceReports)
          .where(and(
            eq(marketMakerPerformanceReports.marketMakerId, profile.id),
            eq(marketMakerPerformanceReports.obligationId, obligation.id),
            eq(marketMakerPerformanceReports.reportDate, today),
          )).limit(1);
        if (existing) continue;

        const snapshots = await db.select().from(marketMakerQuoteSnapshots)
          .where(and(
            eq(marketMakerQuoteSnapshots.marketMakerId, profile.id),
        ...
        generated++;
        results.push({ marketMakerId: profile.id, obligationId: obligation.id, instrument: obligation.instrument, uptimePct });

        const minUptime = parseFloat(obligation.minUptimePct);
        if (uptimePct < minUptime) {
          lowUptimeAlerts++;
          await notifyOwner({
            title: `Low Uptime Alert: Market Maker #${profile.id}`,
            content: `Market Maker #${profile.id} achieved only ${uptimePct.toFixed(1)}% uptime on ${obligation.instrument} (required: ${minUptime}%). Penalty: ₦${penaltyAmount.toLocaleString()}.`,
          });
        }
      }
      ...
      if (generated > 0) {
        await notifyOwner({
          title: `Manual Performance Report Run: ${generated} reports generated`,
          content: `Admin #${ctx.user.id} triggered a manual run. ${generated} report(s) generated, ${lowUptimeAlerts} low-uptime alert(s) sent.`,
        });
      }

      return { generated, lowUptimeAlerts, results, reportDate: today };
    }),
});
