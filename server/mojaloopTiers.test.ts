/**
 * NEXCOM Exchange — mojaloopTiersRouter Unit Tests
 *
...
describe("mojaloopTiersRouter", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("router is defined", () => {
    expect(mojaloopTiersRouter).toBeDefined();
  });
  it("has procedures", () => {
    expect(Object.keys(mojaloopTiersRouter)).not.toHaveLength(0);
  });
});
