# Generic Mission-Critical Codebase Assurance Prompt
...
Do not treat deleting a TODO, renaming/removing a test, suppressing a rule, broadening an allow-list, changing severity, disabling a control, hiding output, or declaring a risk accepted as a fix. Those are new findings unless they implement an approved requirement and are independently justified and verified.

## 12B. Required finding lifecycle

Maintain a remediation ledger and move each finding through these states only with recorded evidence.

| State | Required work | Exit evidence |
|---|---|---|
| `DISCOVERED` | Preserve and reproduce the detector output. | ID, severity, paths/flow, command/output, impact. |
| `TRIAGED` | Identify requirement, root cause, dependencies, affected interfaces/data, and safe fix boundary. | Root cause, blast radius, owner/requirement, acceptance criteria. |
| `IMPLEMENTING` | Correct the complete path, including source, registration, schema/migration, configuration, infrastructure, runbook, and tests as needed. | Reviewable complete diff; no reachable workaround/placeholder. |
| `REGRESSION_PROVEN` | Add/strengthen a test that fails for the pre-fix defect. | Controlled-defect failure and test identity. |
| `RETESTING` | Execute affected static, unit, contract, real integration, E2E, security, concurrency, recovery, migration, and performance tests. | Revision-pinned commands, environment, exit codes, artifacts, durable results. |
| `VERIFIED_FIXED` | Re-run original discovery, review the affected path, and update claim/evidence traceability. | Finding absent, all evidence passes, requirement record updated. |
| `EXTERNAL_BLOCKED` | Use only for unavailable authority/requirements, inaccessible approved dependency/sandbox, third-party change, legal decision, or requester action. | Exact external dependency, minimum unblocking action, risk, and release impact. |

`EXTERNAL_BLOCKED` remains a **BLOCKED** release state. `NOT_APPLICABLE` is valid only for a demonstrably absent component/equivalence role and may not be used to remove a real defect.
...
3. Trace equivalent components and implement the smallest **complete** correction across all necessary source, tests, contracts, migrations, configuration, registration, infrastructure, and runbooks.
4. Prove the fix with a regression test. For funds, security, authorization, privacy, durability, integration, asynchronous, or recovery flows, run real isolated dependencies and public-interface E2E evidence.
5. Re-run the original detector and all affected gates. If the finding persists, a dependent test fails, or a new material issue appears, return to step 2.
6. Repeat until all in-scope entries are `VERIFIED_FIXED`.
7. When an issue is objectively `EXTERNAL_BLOCKED`, continue remediating every independent issue and deliver a BLOCKED status with the precise minimal external action needed. Do not use one blocked issue to stop work on unrelated findings.
...
A status of **RELEASEABLE** is permitted only when every mandatory gate passes and all in-scope findings are `VERIFIED_FIXED`. In strict mode, Medium and Low findings also require a fix and re-test. A waiver is valid only when explicitly requested and approved by the accountable owner, is time-bound and documented with compensating controls, and does not cover Critical funds-integrity, authorization, secret-exposure, data-loss, incomplete-production-path, missing-real-integration, or unverified-audit defects.

Before responding finally, report the remediation-ledger totals by state. If any in-scope finding is not `VERIFIED_FIXED`, respond with a **BLOCKED progress report**, remaining external blockers, and the next remediation actions; never describe a findings-only report as completed assurance.
