...
// consumer groups, LZ4 compression, and graceful fallback to in-memory dispatch
// when the Kafka cluster is unreachable. Preserves the same public interface so
...
// Client wraps segmentio/kafka-go with a producer writer, per-topic consumer
// readers, and graceful in-memory fallback when Kafka is unreachable.
type Client struct {
brokers      []string
connected    bool
fallbackMode bool
...
// Falls back to in-memory dispatch if the Kafka cluster is unreachable.
...
if err != nil {
log.Printf("[Kafka] WARN: Cannot reach %s: %v — running in fallback mode (in-memory dispatch)", c.brokers[0], err)
c.mu.Lock()
c.fallbackMode = true
c.connected = false
c.mu.Unlock()
...
conn.Close()
if metaErr != nil {
log.Printf("[Kafka] WARN: Metadata read failed: %v — running in fallback mode", metaErr)
c.mu.Lock()
c.fallbackMode = true
c.connected = false
c.mu.Unlock()
...
// Produce sends a message to a Kafka topic using the real Kafka protocol.
// Falls back to local handler dispatch when Kafka is unavailable.
...
defer cancel()
if writeErr := writer.WriteMessages(writeCtx, msg); writeErr != nil {
log.Printf("[Kafka] Produce error topic=%s key=%s err=%v — dispatching locally", topic, key, writeErr)
} else {
log.Printf("[Kafka] Produced to topic=%s key=%s size=%d bytes", topic, key, len(data))
}
} else {
log.Printf("[Kafka] Fallback produce: topic=%s key=%s size=%d bytes", topic, key, len(data))
}

// Always dispatch to local handlers for in-process event-driven processing
c.mu.RLock()
