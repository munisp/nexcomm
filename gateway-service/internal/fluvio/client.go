...
// Client wraps the Fluvio Sidecar HTTP API with connection pooling,
// retry logic, circuit breaker, and in-memory fallback.
type Client struct {
sidecarURL   string
...
if err != nil {
if c.connected.Load() {
log.Printf("[Fluvio] Sidecar unreachable (%v) — switching to fallback mode", err)
}
...
c.connected.Store(health.Status == "ok" || health.Status == "degraded")
c.fallbackMode.Store(health.FallbackMode)
...
if err != nil {
c.errors.Add(1)
log.Printf("[Fluvio] Produce error topic=%s: %v (fallback active)", topic, err)
// Dispatch to local consumers as fallback
c.dispatchLocal(topic, body)
...
c.messagesProduced.Add(1)
// Also dispatch to local consumers for in-process subscriptions
c.dispatchLocal(topic, body)
return nil
}
