...
export default function BulkAmendModal({ orders, open, onClose, onAmended }: Props) {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { if (!v) { onClose(); resetForm(); } }}>
      <DialogContent className="max-w-lg bg-background border-border text-white">
        <DialogHeader>
        ...
        <div className="flex items-center gap-2 text-xs mb-4">
          <span className={step === "entry" ? "text-amber-400 font-semibold" : "text-muted-foreground"}>1. Edit</span>
          <ArrowRight className="w-3 h-3 text-gray-600" />
          <span className={step === "confirm" ? "text-amber-400 font-semibold" : "text-muted-foreground"}>2. Confirm</span>
        </div>
        ...
        {step === "entry" && (
          <div className="space-y-4">
            ...
            <div className="bg-card/60 border border-border rounded-lg p-3 max-h-36 overflow-y-auto">
              <p className="text-xs text-muted-foreground mb-2 uppercase tracking-wide font-semibold">Selected Orders</p>
              <div className="space-y-1">
                {orders.map((o, i) => (
                  <div key={o.id} className="flex items-center justify-between text-xs">
                    <span className="font-mono text-muted-foreground">#{o.id} {o.symbol}</span>
            ...
            <div className="space-y-1.5">
              <Label className="text-muted-foreground text-sm">
                New Quantity <span className="text-muted-foreground font-normal">(optional — leave blank to keep current)</span>
              </Label>
              <Input
                type="number"
                min={0.0001}
            ...
            <div className="space-y-1.5">
              <Label className="text-muted-foreground text-sm">
                New Limit Price (₦) <span className="text-muted-foreground font-normal">(optional)</span>
              </Label>
              <Input
                type="number"
                min={0.0001}
            ...
            <div className="space-y-1.5">
              ...
              {!reason.length && (
                <p className="text-[11px] text-muted-foreground">A reason is required to proceed with bulk amendments.</p>
              )}
            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-card/60 border border-border rounded-lg overflow-hidden">
              ...
              <div className="divide-y divide-border">
                <div className="flex justify-between px-3 py-2 text-sm">
                  <span className="text-muted-foreground">Orders to amend</span>
                  <Badge className="bg-amber-600/20 text-amber-400 border-amber-600/30">{validCount}</Badge>
                </div>
                {invalidCount > 0 && (
                  <div className="flex justify-between px-3 py-2 text-sm">
                    <span className="text-muted-foreground">Orders skipped</span>
                ...
                {hasPrice && (
                  <div className="flex justify-between px-3 py-2 text-sm">
                    <span className="text-muted-foreground">New limit price</span>
                ...
                {reason.trim() && (
                  <div className="flex justify-between px-3 py-2 text-sm">
                    ...
                    <span className="text-muted-foreground italic text-right max-w-[60%]">"{reason.trim()}"</span>
                  </div>
                )}
              </div>
            </div>
            ...
            {hasQty && hasPrice && totalNewNotional > 0 && (
              <div className="bg-card/60 border border-border rounded-lg overflow-hidden">
                ...
                <div className="divide-y divide-border">
                  <div className="flex justify-between px-3 py-2 text-sm">
                    <span className="text-muted-foreground">Total new notional</span>
                    <span className="text-white font-mono">{fmtNGN(totalNewNotional)}</span>
                  </div>
                  <div className="flex justify-between px-3 py-2 text-sm">
                    <span className="text-muted-foreground">Est. total fee (0.15% taker)</span>
