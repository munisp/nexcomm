...
const STATUS_COLORS: Record<string, string> = {
  ...
  REPAID: "bg-gray-500/15 text-muted-foreground border-gray-500/30",
  ...
  return (
    <div className="min-h-screen bg-[#0a0e1a] text-white p-6">
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Seeds, fertilizer, equipment &amp; cash loans for farmers</p>
          </div>
        </div>
            ...
            <div className="space-y-4 mt-2">
              ...
              <div>
                ...
                <Input value={form.inputDescription} onChange={e => setForm(f => ({ ...f, inputDescription: e.target.value }))}
                  className="bg-[#0a0e1a] border-border text-white mt-1" placeholder="e.g. NPK 15:15:15 — 50 bags (50kg each)" />
              </div>
              <div>
                <Label>Requested Amount (₦) *</Label>
                <Input type="number" value={form.requestedValueNgn} onChange={e => setForm(f => ({ ...f, requestedValueNgn: e.target.value }))}
                  className="bg-[#0a0e1a] border-border text-white mt-1" placeholder="500000" />
              </div>
              <div>
      ...
      <div className="grid grid-cols-4 md:grid-cols-8 gap-2 mb-8">
        {INPUT_TYPES.map(t => (
          <Card key={t.value} className="bg-[#111827] border-border/50 hover:border-green-500/30 cursor-pointer transition-colors">
            <CardContent className="p-2 text-center">
              <div className="text-xl mb-1">{t.icon}</div>
              <p className="text-xs font-medium text-white leading-tight">{t.label}</p>
              <p className="text-xs text-muted-foreground mt-0.5">{loans.filter(l => l.inputType === t.value).length}</p>
            </CardContent>
          </Card>
        ))}
      </div>
        ...
        <CardContent className="space-y-3">
          {loans.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground font-medium">No financing applications yet</p>
              <p className="text-muted-foreground text-sm mt-1">Apply for seeds, fertilizer, equipment or cash loans</p>
            </div>
          ) : loans.map(loan => {
            ...
            return (
              <div key={loan.id} className="bg-[#0a0e1a] rounded-xl p-4 border border-border/30">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">{loan.inputDescription}</p>
                    <p className="text-xs text-muted-foreground mt-0.5">Applied {new Date(loan.createdAt).toLocaleDateString()}</p>
                  </div>
