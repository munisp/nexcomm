        ...
        <CardContent>
          <div className="flex flex-wrap gap-2">
            {[
              ...
              <div key={c.code} className="flex items-center gap-2 bg-muted/50 rounded-lg px-3 py-2">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <Input
                ...
                className="h-8 w-40 bg-muted border-border text-white text-xs"
              />
              <Button size="sm" variant="outline" onClick={() => refetch()} className="h-8 border-border">
        ...
        <CardContent className="p-0">
          <Table>
            <TableHeader>
              <TableRow className="border-border">
                ...
                <TableHead className="text-muted-foreground text-xs">Service Code</TableHead>
                <TableHead className="text-muted-foreground text-xs">Menu</TableHead>
                <TableHead className="text-muted-foreground text-xs">Status</TableHead>
                <TableHead className="text-muted-foreground text-xs">Interactions</TableHead>
              ...
              {sessions.length === 0 ? (
                ...
                sessions.map((s) => (
                  <TableRow key={s.id} className="border-border hover:bg-muted/30">
                    ...
                    <TableCell className="text-muted-foreground text-xs">{s.currentMenu}</TableCell>
                    <TableCell>
                      <Badge className={`text-xs ${statusColor(s.status)}`}>{s.status}</Badge>
                    </TableCell>
                    <TableCell className="text-muted-foreground text-xs text-center">{s.totalInteractions}</TableCell>
          ...
          {total > 20 && (
            <div className="flex items-center justify-between px-4 py-2 border-t border-border">
              <span className="text-xs text-muted-foreground">{total} total sessions</span>
              <div className="flex gap-2">
                <Button size="sm" variant="outline" disabled={page === 1} onClick={() => setPage(p => p - 1)} className="h-7 text-xs border-border">Prev</Button>
                <Button size="sm" variant="outline" disabled={page * 20 >= total} onClick={() => setPage(p => p + 1)} className="h-7 text-xs border-border">Next</Button>
              </div>
            </div>
          )}
        </CardContent>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <Input
                  ...
                  className="h-7 w-32 bg-muted border-border text-white text-xs"
                />
                <Button size="sm" variant="outline" onClick={() => refetch()} className="h-7 border-border">
                  <RefreshCw className="h-3 w-3" />
                </Button>
              </div>
            </div>
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-border">
                  <TableHead className="text-muted-foreground text-xs">Phone</TableHead>
                  <TableHead className="text-muted-foreground text-xs">Status</TableHead>
                  <TableHead className="text-muted-foreground text-xs">Msgs</TableHead>
              ...
              <TableBody>
                {contacts.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={4} className="text-center text-muted-foreground py-6 text-xs">No contacts yet</TableCell>
                  </TableRow>
                ) : (
                  contacts.map((c) => (
                    <TableRow
                      key={c.id}
                      className={`border-border cursor-pointer hover:bg-muted/30 ${selectedContact === c.id ? "bg-muted/50" : ""}`}
                    ...
                    >
                      ...
                      <TableCell className="text-muted-foreground text-xs">{c.totalMessages}</TableCell>
                      <TableCell>
      ...
      <Dialog open={showMsgDialog} onOpenChange={setShowMsgDialog}>
        <DialogContent className="bg-secondary border-border">
          ...
          <Textarea
            ...
            className="bg-muted border-border text-white min-h-24"
          />
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowMsgDialog(false)} className="border-border">Cancel</Button>
            <Button
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-border">
                  ...
                  <TableHead className="text-muted-foreground text-xs">Verified</TableHead>
                  <TableHead className="text-muted-foreground text-xs">Commands</TableHead>
                  <TableHead className="text-muted-foreground text-xs">Actions</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {contacts.length === 0 ? (
                  ...
                  contacts.map((c) => (
                    ...
                    >
                      ...
                      <TableCell>
                        <Badge className={`text-xs ${c.isVerified ? "bg-emerald-500/20 text-emerald-400" : "bg-slate-600 text-muted-foreground"}`}>
                          {c.isVerified ? "Verified" : "Pending"}
                        </Badge>
                      </TableCell>
                      <TableCell className="text-muted-foreground text-xs">{c.totalCommands}</TableCell>
                      <TableCell>
                        <div className="flex gap-1">
                          <Button
                            size="sm"
...
export default function ChannelDashboard() {
  ...
  return (
    <div className="p-6 space-y-6 bg-card min-h-screen">
      {/* Header */}
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-muted-foreground text-sm">USSD · WhatsApp · Telegram — unified messaging admin</p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
            <div className="flex items-start gap-2">
              ...
              <div>
                ...
                <p className="text-muted-foreground">Africa's Talking gateway · Redis session store · Kafka events · PIN auth · 5-level menu tree</p>
              </div>
            </div>
            <div className="flex items-start gap-2">
              ...
              <div>
                ...
                <p className="text-muted-foreground">Meta Cloud API webhooks · Go channel-gateway · Python NLP intent classifier · 15 command types</p>
              </div>
            </div>
            <div className="flex items-start gap-2">
              ...
              <div>
                ...
                <p className="text-muted-foreground">Telegram Bot API · Go webhook handler · Python command router · Inline keyboards · Markdown</p>
              </div>
            </div>
          </div>
        </CardContent>
