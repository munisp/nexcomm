      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <Label className="text-xs text-muted-foreground mb-1 block">Bid Price</Label>
              <Input
                type="number"
                step="0.0001"
                placeholder="0.0000"
                ...
                className="bg-card border-border text-white h-8 text-sm"
              />
            </div>
            <div>
              <Label className="text-xs text-muted-foreground mb-1 block">Ask Price</Label>
              <Input
                type="number"
                step="0.0001"
                placeholder="0.0000"
            ...
            <div>
              <Label className="text-xs text-muted-foreground mb-1 block">Bid Size</Label>
              <Input
                type="number"
                step="0.01"
                placeholder="0.00"
            ...
            <div>
              <Label className="text-xs text-muted-foreground mb-1 block">Ask Size</Label>
              <Input
                type="number"
                step="0.01"
                placeholder="0.00"
  ...
  if (!profile) {
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center text-center px-6">
        ...
        <p className="text-muted-foreground text-sm mb-6">You need an active market maker profile to record quotes.</p>
  ...
  return (
    <div className="min-h-screen bg-background text-white pb-10">
      ...
      <div className="border-b border-border bg-card/60 backdrop-blur-sm sticky top-0 z-10">
        <div className="max-w-4xl mx-auto px-4 py-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground">{profile.firmName} — {profile.status}</p>
          </div>
          <Button
            size="sm"
            variant="outline"
      ...
      <div className="max-w-4xl mx-auto px-4 py-6 space-y-6">
        ...
        <section>
          <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-3">Active Obligations</h2>
          {!obligations || obligations.length === 0 ? (
            <div className="bg-secondary/30 rounded-xl border border-border/50 p-8 text-center text-muted-foreground">
        ...
        {/* Performance Reports */}
        <section>
          <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-3">Recent Performance Reports</h2>
          {reportsLoading ? (
                ...
                <tbody>
                  {reports.map((r) => (
                    <tr key={r.id} className="border-b border-border/30 hover:bg-muted/20">
                      ...
                      <td className="p-3 text-right">
                        <span className={`font-medium ${((r.totalSnapshots ?? 0) - (r.compliantSnapshots ?? 0)) > 0 ? "text-red-400" : "text-muted-foreground"}`}>
        ...
        {reports && reports.length > 0 && (
          <section>
            <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-3">Compliance Summary</h2>
            <div className="grid grid-cols-3 gap-3">
              <Card className="bg-secondary/40 border-border">
                <CardContent className="p-4 text-center">
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">Days Compliant</p>
                </CardContent>
              </Card>
              <Card className="bg-secondary/40 border-border">
                <CardContent className="p-4 text-center">
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">Avg Spread (30d)</p>
                </CardContent>
              </Card>
              <Card className="bg-secondary/40 border-border">
                <CardContent className="p-4 text-center">
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">Total Breaches</p>
                </CardContent>
              </Card>
            </div>
          </section>
